...
public:
    ...
    {
        doMatrix(c, "Hello, world!", &self::doDeflate1_zlib);
        doMatrix(c, "Hello, world!", &self::doDeflate2_zlib);
        log << "no-silence keepalive" << std::endl;
        doMatrix(c, corpus1(56), &self::doDeflate2_zlib);
        doMatrix(c, corpus1(1024), &self::doDeflate1_zlib);
    }
...
TEST_SUITE(deflate_stream_test, "zlib.deflate_stream");
